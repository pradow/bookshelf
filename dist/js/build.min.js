(function(exports){const events=function(){let events={};function on(eventName,fn){events[eventName]=events[eventName]||[];events[eventName].push(fn)}function off(eventName,fn){if(events[eventName]){for(let i=0;i<events[eventName].length;i++){if(events[eventName][i]===fn){events[eventName].splice(i,1);break}}}}function emit(eventName,data){if(events[eventName]){events[eventName].forEach(fn=>{fn(data)})}}return{on:on,off:off,emit:emit}}();const userDataFunctions=function(){let userData={bookIdArr:[],booksData:[]};if(!localStorage.hasOwnProperty("bookshelfApp")){updateLocalStorage()}else{userData=JSON.parse(localStorage.getItem("bookshelfApp"))}function updateLocalStorage(){localStorage.setItem("bookshelfApp",JSON.stringify(userData));events.emit("userDataChange",userData)}function addBook(bookInfo){userData={...userData,bookIdArr:[...userData.bookIdArr,bookInfo.id],booksData:[...userData.booksData,bookInfo]};console.log(userData);updateLocalStorage()}function removeBook(bookId){userData={...userData,bookIdArr:userData.bookIdArr.filter(item=>item!==bookId),booksData:userData.booksData.filter(item=>item.id!==bookId)};updateLocalStorage()}events.on("addBook",addBook);events.on("removeBook",removeBook);return{userData:userData}}();const search=function(){if(document.querySelectorAll(".js-book-search")){const body=document.querySelector("body");const bookSearchForm=document.querySelectorAll(".js-book-search");const bookSearchInput=document.querySelector("#bookSearch");const apiUrl="http://openlibrary.org/search.json?q=";async function fetchData(url){pageStateChange("search");try{const response=await fetch(url);const data=await response.json();console.log(data);events.emit("searchDataChange",data)}catch(error){console.error(error)}}function searchBooks(e){e.preventDefault();const searchInput=e.target.querySelector("input");if(!searchInput.value.replace(/ /g,"")){return}const searchValue=searchInput.value.replace(/ /g,"+");const data=fetchData(apiUrl+searchValue);console.log("searching...")}function pageStateChange(page){const pagesArray=["home","search","results","bookshelf"];pagesArray.forEach(item=>{if(item!==page){body.classList.remove(item)}else{body.classList.add(item)}})}bookSearchForm.forEach(item=>item.addEventListener("submit",searchBooks));return{pageStateChange:pageStateChange}}}();const results=function(){if(document.querySelector(".js-results")){const body=document.querySelector("body");const resultsContainer=document.querySelector(".js-results");let userData=userDataFunctions.userData;function updateButton(buttonClicked,isAdding){if(isAdding){buttonClicked.innerText="Remove from bookshelf"}else{buttonClicked.innerText="Add to bookshelf"}}function emitRemoveBook(e){const buttonClicked=e.currentTarget;const parentEl=e.currentTarget.parentNode.parentNode.parentNode.parentNode;updateButton(buttonClicked,false);events.emit("removeBook",parentEl.dataset.bookid)}function emitAddBook(e){const buttonClicked=e.currentTarget;const parentEl=e.currentTarget.parentNode.parentNode.parentNode;const bookInfo={id:parentEl.dataset.bookid,title:parentEl.dataset.booktitle,author:parentEl.dataset.bookauthor,cover:parentEl.dataset.bookcover};updateButton(buttonClicked,true);events.emit("addBook",bookInfo)}function populateData(data){let htmlToAppend="";data.docs.forEach(item=>{const itemInfo={id:item.key.slice(7),title:item.title,author:`${item.author_name?item.author_name[0]:""}`,cover:`${item.cover_edition_key?`https://covers.openlibrary.org/b/olid/${item.cover_edition_key}-L.jpg`:"https://openlibrary.org/images/icons/avatar_book-lg.png"}`};htmlToAppend+=createBookEl(itemInfo)});resultsContainer.innerHTML=htmlToAppend;document.querySelectorAll(".js-addRemovebook").forEach(item=>{if(item.classList.contains("add-button")){item.addEventListener("click",emitAddBook)}else{item.addEventListener("click",emitRemoveBook)}});search.pageStateChange("results")}function createBookEl(item){return`<a class="book-wrapper" data-bookid="${item.id}" data-booktitle="${item.title}" data-bookauthor="${item.author}" data-bookcover="${item.cover}">
      ...
      </a>`}function updateUserData(data){userData=data}events.on("searchDataChange",populateData);events.on("userDataChange",updateUserData);return{createBookEl:createBookEl}}}();const bookshelf=function(){if(document.querySelector(".js-results")){const body=document.querySelector("body");const bookshelfContainer=document.querySelector(".js-results");const bookshelfLink=document.querySelector('[data-pageLink="bookshelf"]');let userData=userDataFunctions.userData;function emitRemoveBook(e){const buttonClicked=e.currentTarget;const parentEl=buttonClicked.parentNode.parentNode.parentNode;console.log(bookshelfContainer);bookshelfContainer.removeChild(parentEl);events.emit("removeBook",parentEl.dataset.bookid)}function populateData(data){let htmlToAppend="";if(data.length<=0){bookshelfContainer.innerHTML=`<p>Your bookshelf is empty`;return}data.forEach(item=>{htmlToAppend+=results.createBookEl(item)});bookshelfContainer.innerHTML=htmlToAppend;document.querySelectorAll(".js-addRemovebook").forEach(item=>{item.addEventListener("click",emitRemoveBook)});search.pageStateChange("bookshelf")}bookshelfLink.addEventListener("click",e=>{e.preventDefault();populateData(userData.booksData)});function updateUserData(data){userData=data}events.on("userDataChange",updateUserData)}}()})(typeof window=="undefined"?window={}:window);